...
function selectNode(d) {
  ...
  if (pathData) {
    var txs = d3.selectAll('.link').filter(function (d) {
      return pathData.indexOf(d.source.tx) > -1 && pathData.indexOf(d.target.tx) > -1;
    });

    txs.classed('selected', true).moveToFront().transition().duration(250);

    var descendants = d3.selectAll('.node').filter(function (d) {
      return pathData.indexOf(d.tx) > -1;
    });

    descendants.classed('selected', true).moveToFront().transition().duration(250);

    // unhide balances
    descendants.selectAll('.hid').style('display', 'inherit');

    var pDataArray = d3.selectAll('.link').filter(function (d) {
      return pathData.indexOf(d.source.tx) > -1 && pathData.indexOf(d.target.tx) > -1;
    }).data();
    var totalLength = d3.sum(pDataArray, function (d) {
      return d.amount;
    });
